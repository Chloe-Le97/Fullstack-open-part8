  ...
  Query: {
    ...
    allBooks: (roots,args) => {
        if(args.author&&args.genre){
            let newBook = [];
            const bookAuthor = books.filter(p=>p.author===args.author);
            bookAuthor.map(item=> {
                if(item.genres.find(p=>p===args.genre)){newBook.push(item)}
            })
            return newBook
  ...
  Mutation:{
      addBook: (root,args) => {
          if(!authors.find(p=>p.name===args.author)){
            ...
            const author ={name:authorName,id:uuid()}
